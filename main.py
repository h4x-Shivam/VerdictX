...
â•‘         StockAI â€” NSE-First Backend                             â•‘
â•‘                                                                  â•‘
â•‘  Data pipeline (priority order):                                â•‘
â•‘  1. NSE official web endpoints  (primary, free, no key)         â•‘
â•‘  2. yfinance .NS suffix         (fallback for fundamentals)     â•‘
â•‘  3. jugaad-data                 (optional fallback)             â•‘
â•‘                                                                  â•‘
â•‘  News: Google News RSS â†’ MoneyControl, ET, Mint, BS, NDTV      â•‘
â•‘                                                                  â•‘
â•‘  SETUP:                                                          â•‘
â•‘    pip install requests yfinance feedparser ollama              â•‘
â•‘    pip install jugaad-data   (optional)                         â•‘
â•‘                                                                  â•‘
â•‘  OPTIONAL PAID KEY (RapidAPI NSE providers):                    â•‘
â•‘    Create config.py â†’ RAPIDAPI_KEY = "your_key_here"           â•‘
â•‘    System works fully without it.                               â•‘
â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
"""

"""
StockAI â€” Backend Engine
------------------------
Fetches real NSE data, runs multi-agent AI analysis,
and tracks prediction accuracy over time.

Data sources (in order of preference):
  1. NSE web endpoints via jugaad-data  â†’  live prices, VWAP, delivery %
  2. yfinance (.NS suffix)              â†’  PE, margins, revenue, EPS etc.
  3. jugaad fallback                    â†’  PE/PB when yfinance is dry

News comes from Google News RSS, filtered to MoneyControl,
Economic Times, Mint, Business Standard, and a few others.

Setup:
    pip install jugaad-data yfinance feedparser requests ollama plotly
    ollama pull mistral
"""
"""
â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
...
StockAI â€” Backend Engine
------------------------
NSE-first data pipeline + multi-agent AI analysis + accuracy tracking.

Fixes in this version:
  1. News relevance   â€” strict title-level filtering, no body bleed
  2. Price change     â€” uses prev_close from yfinance as reliable fallback
  3. Fair value       â€” analyst target wins; sector PE uses actual sector,
                        not yfinance's often-wrong classification
  4. Predictions      â€” show "Awaiting X more days" with baseline price
  5. Sector PE map    â€” expanded + NSE industry name matching

Data priority:
  Price/volume  â†’ NSE endpoints via jugaad-data (live)
  Fundamentals  â†’ yfinance .NS (delayed but complete)
  PE/PB backup  â†’ jugaad live_stock_data

News: Google News RSS â†’ MoneyControl, ET, Mint, BS, NDTV Profit
      Strict title-level relevance filter applied after fetch.

Setup:
    pip install jugaad-data yfinance feedparser requests ollama plotly reportlab
    ollama pull mistral
"""

"""
StockAI — Backend Engine
------------------------
NSE-first data pipeline + multi-agent AI analysis + accuracy tracking.

Fixes in this version:
  - Rupee symbol encoding: use ASCII-safe ₹ via unicode escape u20b9
  - Signal breakdown: no special Unicode chars (x instead of ×)
  - Decision engine: composite weighted score with breakdown
  - PDF: graceful fallback if reportlab not installed

Data priority:
  Price/volume  → NSE endpoints via jugaad-data (live)
  Fundamentals  → yfinance .NS (delayed but complete)
  PE/PB backup  → jugaad live_stock_data
"""
...
# ─────────────────────────────────────────────────────────────────
# PDF Report — graceful fallback if reportlab not installed
# ─────────────────────────────────────────────────────────────────

def generate_pdf_report(ticker: str, data: dict, sentiment: dict,
                        bull: dict, bear: dict, verdict: dict,
                        news_items: list, decision_engine: dict = None) -> bytes:
    try:
    ...
    DARK   = colors.HexColor("#0f1120")
    GREEN  = colors.HexColor("#00e5a0")
    RED_C  = colors.HexColor("#ff3d6b")
    YELLOW = colors.HexColor("#ffc400")
    ACCENT = colors.HexColor("#6366f1")
    LGREY  = colors.HexColor("#f5f6fb")
    MGREY  = colors.HexColor("#7c85a8")
    WHITE  = colors.white
    ...
    S = {
        "title":  sty("T",  fontSize=20, fontName="Helvetica-Bold", textColor=DARK, spaceAfter=4),
        "sub":    sty("S",  fontSize=10, textColor=MGREY, spaceAfter=14),
        "h1":     sty("H1", fontSize=12, fontName="Helvetica-Bold", textColor=DARK,
                      spaceBefore=12, spaceAfter=5),
        "body":   sty("B",  fontSize=9,  textColor=DARK, leading=14, spaceAfter=3),
        "small":  sty("Sm", fontSize=8,  textColor=MGREY, spaceAfter=2),
    }
    ...
    # Cover
    story.append(Paragraph("StockAI Research Report", S["sub"]))
    story.append(Paragraph(info.get("company_name", ticker), S["title"]))
    story.append(Paragraph(
        f"NSE: {info.get('nse_symbol', ticker)}  |  {info.get('sector', 'N/A')}  |  "
        f"Generated {datetime.now().strftime('%d %b %Y %I:%M %p')}  |  "
        f"Source: {info.get('data_source', '')}",
        S["small"]
    ))
    hr()

    # Verdict banner
    vv  = verdict.get("verdict", "HOLD")
    cf  = verdict.get("confidence", 50)
    rsk = verdict.get("risk", "MEDIUM")
    tf  = verdict.get("timeframe", "short-term")
    vc  = GREEN if vv == "BUY" else RED_C if vv == "SELL" else YELLOW

    banner = [[
        Paragraph(vv, ParagraphStyle("VB", fontSize=20, fontName="Helvetica-Bold",
                                     textColor=vc, alignment=1)),
        Paragraph(f"{cf}%\nConfidence",
                  ParagraphStyle("CB", fontSize=13, fontName="Helvetica-Bold",
                                 textColor=DARK, alignment=1, leading=16)),
        Paragraph(f"{rsk}\nRisk",
                  ParagraphStyle("RB", fontSize=13, fontName="Helvetica-Bold",
                                 textColor=DARK, alignment=1, leading=16)),
        Paragraph(f"{tf.title()}\nTimeframe",
                  ParagraphStyle("TB", fontSize=11, textColor=DARK, alignment=1, leading=16)),
    ]]
    if decision_engine:
        banner[0].append(Paragraph(
            f"{decision_engine['composite']}\nComposite",
            ParagraphStyle("DE", fontSize=13, fontName="Helvetica-Bold",
                           textColor=DARK, alignment=1, leading=16)
        ))

    bt = Table(banner, colWidths=["20%"] * len(banner[0]))
    ...
    story.append(bt); sp(10)

    story.append(Paragraph("AI Analysis Summary", S["h1"]))
    story.append(Paragraph(verdict.get("final_reasoning", ""), S["body"]))
    if verdict.get("key_catalyst"):
        story.append(Paragraph(f"<b>Key Catalyst:</b> {verdict['key_catalyst']}", S["body"]))
    if verdict.get("key_risk"):
        story.append(Paragraph(f"<b>Key Risk:</b> {verdict['key_risk']}", S["body"]))
    story.append(Paragraph(verdict.get("signal_breakdown", ""), S["small"]))
    hr()

    # Decision Engine breakdown
    if decision_engine:
        story.append(Paragraph("Decision Engine Scores", S["h1"]))
        story.append(Paragraph(
            f"Composite Score: {decision_engine['composite']} -> {decision_engine['signal']}",
            S["body"]
        ))
        de_rows = [["Signal", "Score", "Weight"]]
        for comp in decision_engine["components"].values():
            de_rows.append([comp["label"], str(comp["score"]), f"{comp['weight']}%"])
        de_rows.append(["Data Completeness", f"{decision_engine['completeness']}%", ""])
        de_t = Table(de_rows, colWidths=["50%", "25%", "25%"])
        de_t.setStyle(TableStyle([
            ...
            ("GRID",          (0,0),(-1,-1), 0.3, colors.HexColor("#dde0ef")),
    ...
    # Price table
    story.append(Paragraph("Price and Valuation", S["h1"]))
    cur = raw.get("currentPrice")
    pch = raw.get("pchange", 0) or 0
    rows = [
        ["Metric", "Value"],
        ["Current Price",     fmt_rupee(cur)],
        ["Change Today",      f"{pch:+.2f}%"],
        ["Previous Close",    fmt_rupee(raw.get("prevClose"))],
        ["52W High / Low",    f"{fmt_rupee(raw.get('week52High'))} / {fmt_rupee(raw.get('week52Low'))}"],
        ["Fair Value (Est.)", f"{fmt_rupee(fv_p.get('value'))} - {fv_p.get('method', '')}"],
        ["Upside / Downside", f"{fv_p.get('upside', 0):+.2f}%"],
        ["P/E (TTM)",         str(raw.get("trailingPE", "N/A"))],
        ["Market Cap",        fmt_cr(raw.get("marketCap"))],
        ["Delivery %",        f"{raw.get('deliveryPct', 'N/A')}%"],
    ]
    _add_table(story, rows, ACCENT, LGREY, WHITE)
    hr()

    # Financials
    story.append(Paragraph("Financial Metrics", S["h1"]))
    fin = [
        ["Metric", "Value"],
        ["Revenue (TTM)",  fmt_cr(info.get("total_revenue"))],
        ["Revenue Growth", fmt_pct(raw.get("revenueGrowth"))],
        ["Profit Margins", fmt_pct(raw.get("profitMargins"))],
        ["Free Cash Flow", fmt_cr(info.get("free_cash_flow"))],
        ["Debt / Equity",  str(raw.get("debtToEquity", "N/A"))],
        ["ROE",            fmt_pct(raw.get("returnOnEquity"))],
        ["EPS (TTM)",      fmt_rupee(info.get("trailing_eps"))],
        ["Beta",           str(raw.get("beta", "N/A"))],
    ]
    _add_table(story, fin, ACCENT, LGREY, WHITE)

    story.append(PageBreak())

    # Sentiment
    sv = sentiment.get("sentiment", "N/A")
    ss = sentiment.get("score", 0)
    sc_color = GREEN if sv == "POSITIVE" else RED_C if sv == "NEGATIVE" else YELLOW
    story.append(Paragraph("Market Sentiment", S["h1"]))
    story.append(Paragraph(
        f"<b>{sv}</b>  (score: {ss:+d})",
        ParagraphStyle("SV", fontSize=11, fontName="Helvetica-Bold", textColor=sc_color)
    ))
    sp(4)
    ...
    # Bull
    story.append(Paragraph("Bull Case", S["h1"]))
    story.append(Paragraph(bull.get("bull_thesis", ""), S["body"]))
    sp(4)
    for pt in bull.get("bull_points", []):
        icon = {"STRONG": "***", "MODERATE": "**", "WEAK": "*"}.get(pt.get("strength",""), "-")
        story.append(Paragraph(f"<b>{icon} {pt.get('point','')}</b>",
                               ParagraphStyle("BP", fontSize=9,
                                              textColor=colors.HexColor("#007a5e"),
                                              leading=13, spaceAfter=1)))
        story.append(Paragraph(
            f"Cited: {pt.get('metric_cited','')}  |  {pt.get('impact','')}",
            S["small"]
        ))
        sp(3)
    hr()
    ...
    # Bear
    story.append(Paragraph("Bear Case", S["h1"]))
    story.append(Paragraph(bear.get("bear_thesis", ""), S["body"]))
    sp(4)
    for pt in bear.get("bear_points", []):
        icon = {"SEVERE": "!!!", "MODERATE": "!!", "MINOR": "!"}.get(pt.get("severity",""), "-")
        story.append(Paragraph(f"<b>{icon} {pt.get('point','')}</b>",
                               ParagraphStyle("BRP", fontSize=9,
                                              textColor=colors.HexColor("#b00030"),
                                              leading=13, spaceAfter=1)))
        story.append(Paragraph(
            f"Cited: {pt.get('metric_cited','')}  |  {pt.get('impact','')}",
            S["small"]
    ...
    # News
    story.append(Paragraph("Recent News", S["h1"]))
    for item in news_items[:6]:
        story.append(Paragraph(f"<b>{item.get('title','')}</b>",
                               ParagraphStyle("NH", fontSize=8.5, textColor=DARK,
                                              leading=13, spaceAfter=1)))
        story.append(Paragraph(
            f"{item.get('source','')}  |  {item.get('age_label', item.get('published',''))}",
            S["small"]
        ))
        sp(4)
...
def _add_table(story, rows, header_bg, alt_bg, header_text):
